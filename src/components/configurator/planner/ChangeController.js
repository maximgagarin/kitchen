  ...
  return pos;
     ...
     changeSelected(modelType, width) {
       ...
       const rules = {
        ...
        const cabinetName = `${modelType}-${width * 1000}`;
        ...
        modulesArray.sort((a,b)=>a.root.position[rule.axis] - b.root.position[rule.axis]);

        const indexOld = modulesArray.findIndex(m => m.root.uuid === oldInstance.root.uuid);
        ...
        const newPos = this.getPosition(leftModule, rightModule, width, position, rule.axis);

        console.log('newPos', newPos)

          
        const index = plannerConfig.models.findIndex((m) => m.root.uuid === oldInstance.root.uuid);
        if (index !== -1) {
        ...
        testInstance.root.position[rule.axis] = newPos
